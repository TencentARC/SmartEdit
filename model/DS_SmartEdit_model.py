...
from model.two_way_transformer import TwoWayTransformer, PositionEmbeddingRandom
# unet with more input
...
########################################################################################################################################################################
# new
class SmartEdit_model(LlamaPreTrainedModel):
    # _keys_to_ignore_on_load_missing = [r"lm_head.weight"]
    def __init__(self, config):
        super(SmartEdit_model, self).__init__(config)
    ...
    def init_sd_vae_unet(self, model_name_or_path="runwayml/stable-diffusion-v1-5"):
        self.unet = UNet2DConditionModel_ZeroConv.from_pretrained(
    ...
    #############################################################################################################################
    # 4. initialize BIM module
    def init_BIM_module(self):
        embed_dim = 1024
        self.modulate_head = nn.Linear(768, embed_dim)
        self.modulate_transformer = TwoWayTransformer(
            depth=2,
        ...
        self.dimension_reduction_head_zeroconv = zero_module(nn.Conv2d(embed_dim, 320, kernel_size=1, padding=0))
        self.dimension_reduction_head_qformer_out = zero_module(nn.Linear(embed_dim, 768))
        print('BIM module is initialized...')

    #############################################################################################################################
    # 5. load pretrained checkpoint from MLLMSD-11 without BIM module
    def load_pretrain_MLLMSD11(self, pretrained_LLaMA, pretrained_model, pretrained_unet, LLaVA_00002_weights):
        """ 5.1. pretrained LLM with LoRA """
        ...
        """ 5.2. pretrained checkpoint for mm_projector + lm_head + SD_Query and SD_Qformer """
        weights = torch.load(pretrained_model, map_location="cpu")
        ...
        # 1. vec2word: Linear(in_features=4096, out_features=32008, bias=False)
        ...
        """ 5.3. pretrained unet checkpoint """
        unet_ckpt = pretrained_unet
    ...
    ########################################################################################################################################################################
    # inference 3.with BIM module
    def load_BIM_module_for_inference(self, modulate_path):
        weights = torch.load(modulate_path, map_location="cpu")
        modulate_head_param = {
            'weight': weights.pop("modulate_head.weight"),
            'bias': weights.pop("modulate_head.bias")
        }
        self.modulate_head.load_state_dict(modulate_head_param, strict=True)
        # load the pe_layer
        self.pe_layer.positional_encoding_gaussian_matrix = weights.pop(
            "pe_layer.positional_encoding_gaussian_matrix")
        # load the dimension_reduction_head
        dimension_reduction_head_zeroconv_param = {
            'weight': weights.pop("dimension_reduction_head_zeroconv.weight"),
            'bias': weights.pop("dimension_reduction_head_zeroconv.bias")
        }
        self.dimension_reduction_head_zeroconv.load_state_dict(dimension_reduction_head_zeroconv_param)
        # load the dimension_reduction_head_qformer_out
        dimension_reduction_head_qformer_out_param = {
            'weight': weights.pop("dimension_reduction_head_qformer_out.weight"),
            'bias': weights.pop("dimension_reduction_head_qformer_out.bias")
        }
        self.dimension_reduction_head_qformer_out.load_state_dict(dimension_reduction_head_qformer_out_param)
        # load the modulate_transformer
        self.modulate_transformer.load_state_dict({k[len('modulate_transformer.'):]: v for k, v in weights.items()})

    ########################################################################################################################################################################
    # inference 4.with LLM
    def inference_llm(
        self,
    ...
    ########################################################################################################################################################################
    # inference 6.inference with BIM
    def inference_BIM_module(self, clip_image_feature, query_output, latent_w=32, latent_h=32):
        batch_size = clip_image_feature.shape[0]
        query_output_modulate_head = self.modulate_head(query_output)
        # [B, N, C] -> [B, C, H, W]
        clip_image_feature = clip_image_feature.permute(0, 2, 1).reshape(batch_size, 1024, 16, 16)
        clip_image_pe = self.pe_layer((16, 16)).unsqueeze(0).expand(batch_size, -1, -1, -1)
        # Run the transformer
        modulated_SD_features, modulated_image_feature = self.modulate_transformer(clip_image_feature, clip_image_pe,
                                                                                   query_output_modulate_head)
        # [B, H*W, C] -> [B, H, W, C]
        modulated_image_feature = modulated_image_feature.transpose(1, 2).view(batch_size, 1024, 16, 16)
        # upsampling: 16 -> 32
        modulated_image_feature = F.interpolate(modulated_image_feature, size=(latent_w, latent_h), mode="bilinear", align_corners=False)
        # [bs, CLIP_dim=1024, H=16, W=16], [bs, CLIP_dim=1024, H'=32, W'=32]
        modulated_image_feature = self.dimension_reduction_head_zeroconv(modulated_image_feature)
        # [bs, SD_convin_dim=320, H=32, W=32]
        modulated_SD_features = self.dimension_reduction_head_qformer_out(modulated_SD_features)
        # [bs, CLIP_model_max_length=77, CLIP_test_dim=768]
        return modulated_image_feature, modulated_SD_features
    ...
    def forward(
        ...
        CLIP_image_features = self.vision_tower(original_img)
        CLIP_image_features_llm_input = self.mm_projector(CLIP_image_features)
        ...
        LLM_new_attention_masks = LLM_new_attention_masks.to(torch.bfloat16)
        LLM_new_inputs_embeds = LLM_new_inputs_embeds.to(torch.bfloat16)
        ...
        """ 7. BIM module part """
        # Modulate the clip image feature with the query output
        clip_image_feature = CLIP_image_features.to(torch.float32)
        # [bs, mm_projection_length, CLIP_dim]

        query_output_modulate_head = self.modulate_head(query_output)
        # [bs, CLIP_model_max_length, CLIP_dim]

        # [B, N, C] -> [B, C, H, W]
        clip_image_feature = clip_image_feature.permute(0, 2, 1).reshape(batch_size, 1024, 16, 16)
        clip_image_pe = self.pe_layer((16, 16)).unsqueeze(0).expand(batch_size, -1, -1, -1)
        # [bs, 1024, 16, 16], [bs, 1024, 16, 16]

        # Run the transformer
        modulated_SD_feature, modulated_image_feature = self.modulate_transformer(
            image_embedding=clip_image_feature,
            image_pe=clip_image_pe,
            point_embedding=query_output_modulate_head)
        modulated_SD_feature = self.dimension_reduction_head_qformer_out(modulated_SD_feature)
        query_output = query_output + 0.5 * modulated_SD_feature
        # [bs, CLIP_model_max_length, CLIP_dim] -> [bs, CLIP_model_max_length, CLIP_test_dim], [bs, mm_projection_length, CLIP_dim]

        # [B, H*W, C] -> [B, H, W, C]
        modulated_image_feature = modulated_image_feature.transpose(1, 2).view(batch_size, 1024, 16, 16)
        # upsampling: 16 -> 32
        modulated_image_feature = F.interpolate(modulated_image_feature, size=(32, 32), mode="bilinear",
                                                align_corners=False)
        # [bs, 1024, H=32, W=32]
        modulated_image_feature = self.dimension_reduction_head_zeroconv(modulated_image_feature)
        # [bs, SD_convin_dim=320, H=32, W=32]
        ...
        # 8. Diffusion loss: Convert images to latent space
        ...
        # Predict the noise residual -> the BIM output also goes into the unet
        model_pred = self.unet(concatenated_noisy_latents, timesteps, query_output,
                               two_way_image_features_zero_conv=modulated_image_feature).sample
